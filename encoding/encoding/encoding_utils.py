...
def preprocess_features(stories, text_feat, audio_feat, modality, trim, ndelays, use_pca=False, n_comps=0.90):
    ...
    Args:
        ...
        n_comps (float): Target explained variance for PCA (e.g., 0.90).
    ...
    if use_pca:
        ...
        print(f"Text features: {n_components_text} components selected for {n_comps:.2f} components or explained variance")
        text_concat_z = pca_text.transform(text_concat_z)
        
        print(f"Applying PCA to audio features with explained variance threshold: {n_comps}")
        pca_audio = PCA(n_components=n_comps)
        pca_audio.fit(audio_concat_z)
        n_components_audio = pca_audio.n_components_
        print(f"Audio features: {n_components_audio} components selected for {n_comps:.2f} components or explained variance")

...
def ridge_cv_debug(stim, resp, alphas, story_ids, nboots=25, nsplits=5,
    ...
    with_replacement : boolean, default False
        Sample stories with replacement in LOPO bootstrap (if optimize_alpha=True).
    ...
    """
    # Validate inputs
    stim = np.asarray(stim).astype(np.float32)
    resp = np.asarray(resp).astype(np.float32)
    story_ids = np.asarray(story_ids)
    if stim.shape[0] != resp.shape[0]:
        raise ValueError(f"stim and resp must have same number of time points: {stim.shape[0]} vs {resp.shape[0]}")
    
    # Debug: Log input shapes and check for NaN/Inf
    logger.info(f"Input shapes: stim={stim.shape}, resp={resp.shape}, story_ids={story_ids.shape}") if logger else None
    if np.any(np.isnan(stim)) or np.any(np.isinf(stim)):
        logger.warning(f"NaN/Inf in stim: {np.sum(np.isnan(stim))} NaNs, {np.sum(np.isinf(stim))} Infs") if logger else None
    if np.any(np.isnan(resp)) or np.any(np.isinf(resp)):
        logger.warning(f"NaN/Inf in resp: {np.sum(np.isnan(resp))} NaNs, {np.sum(np.isinf(resp))} Infs") if logger else None
    if np.any(np.isnan(story_ids)) or np.any(np.isinf(story_ids)):
        logger.warning(f"NaN/Inf in story_ids: {np.sum(np.isnan(story_ids))} NaNs, {np.sum(np.isinf(story_ids))} Infs") if logger else None
    
    # Debug: Check variance
    stim_var = np.var(stim, axis=0)
    resp_var = np.var(resp, axis=0)
    zero_var_stim = np.sum(stim_var == 0)
    zero_var_resp = np.sum(resp_var == 0)
    if zero_var_stim > 0:
        logger.warning(f"Found {zero_var_stim} features with zero variance in stim") if logger else None
    if zero_var_resp > 0:
        logger.warning(f"Found {zero_var_resp} voxels with zero variance in resp") if logger else None

    # Z-score inputs if requested
    stim = zs(stim) if normalize_stim else stim
    resp = zs(resp) if normalize_resp else resp
    if np.any(np.isnan(stim)):
        logger.warning(f"NaN in stim after z-scoring: {np.sum(np.isnan(stim))} NaNs") if logger else None
    if np.any(np.isnan(resp)):
        logger.warning(f"NaN in resp after z-scoring: {np.sum(np.isnan(resp))} NaNs") if logger else None

    if story_ids is None:
        raise ValueError("story_ids required for group CV and bootstrapping.")
    
    unique_stories = np.unique(story_ids)
    n_stories = len(unique_stories)
    logger.info(f"Number of unique stories: {n_stories}") if logger else None
    ...
    if optimize_alpha:
        ...
        np.random.seed(42)
        story_choices = (np.random.choice(unique_stories, size=nboots, replace=True)
                         if with_replacement else
                         np.random.permutation(unique_stories)[:min(nboots, n_stories)])
        
        def _bootstrap_iter(val_story, iteration, total):
            logger.info(f"Bootstrap iteration {iteration+1}/{total} for story {val_story}...") if logger else None
            heldinds = story_ids == val_story
            notheldinds = ~heldinds
            RRstim, PRstim = stim[notheldinds, :], stim[heldinds, :]
            RRresp, PRresp = resp[notheldinds, :], resp[heldinds, :]
            logger.info(f"Bootstrap {iteration+1}: Train stim={RRstim.shape}, Test stim={PRstim.shape}, Train resp={RRresp.shape}, Test resp={PRresp.shape}") if logger else None
            # Check test response variance
            prresp_var = np.var(PRresp, axis=0)
            zero_var_prresp = np.sum(prresp_var == 0)
            if zero_var_prresp > 0:
                logger.warning(f"Bootstrap {iteration+1}: Found {zero_var_prresp} voxels with zero variance in PRresp") if logger else None
            ...
            if np.any(np.isnan(corrs)):
                logger.warning(f"Bootstrap {iteration+1}: NaN correlations detected: {np.sum(np.isnan(corrs))}") if logger else None
            logger.info(f"Bootstrap {iteration+1}: Mean corr={np.nanmean(corrs):.5f}, Max corr={np.nanmax(corrs):.5f}") if logger else None
            return corrs
        
        bootstrap_results = Parallel(n_jobs=n_jobs)(
            delayed(_bootstrap_iter)(val_story, i, nboots)
            for i, val_story in enumerate(story_choices)
        )
        
        bootstrap_corrs = np.dstack(bootstrap_results) if nboots > 0 else None
        if bootstrap_corrs is not None:
            meanbootcorrs = np.nanmean(bootstrap_corrs, axis=2)  # Use nanmean to handle NaNs
            bestalphainds = np.argmax(meanbootcorrs, axis=0)
            valphas = alphas[bestalphainds]
            logger.info(f"Mean bootstrap correlations shape: {meanbootcorrs.shape}") if logger else None
            for ua in np.unique(valphas):
                sel_vox = np.nonzero(valphas == ua)[0]
                mean_corr = np.nanmean(meanbootcorrs[bestalphainds[sel_vox], sel_vox]) if len(sel_vox) > 0 else 0
    ...
    if nsplits > 0:
        ...
        def _cv_iter(fold_idx, train_idx, test_idx):
            ...
            # Check test response variance
            presp_var = np.var(Presp, axis=0)
            zero_var_presp = np.sum(presp_var == 0)
            if zero_var_presp > 0:
                logger.warning(f"CV fold {fold_idx+1}: Found {zero_var_presp} voxels with zero variance in Presp") if logger else None
            ...
            if np.any(np.isnan(corrs)):
                logger.warning(f"CV fold {fold_idx+1}: NaN correlations detected: {np.sum(np.isnan(corrs))}") if logger else None
            logger.info(f"CV fold {fold_idx+1}: Mean corr={np.nanmean(corrs):.5f}, Max corr={np.nanmax(corrs):.5f}") if logger else None
            return corrs
        ...
        if fold_corrs.size > 0:
            corrs = np.nanmean(fold_corrs, axis=1)  # Use nanmean to handle NaNs
            logger.info(f"Fold correlations shape: {fold_corrs.shape}") if logger else None
            logger.info("Completed CV: mean correlation across voxels=%0.5f, max=%0.5f",
                        np.nanmean(corrs), np.nanmax(corrs)) if logger else None
        else:
            corrs = np.array([])
...
def ridge_cv(stim, resp, alphas, story_ids, nboots=50, nsplits=50,
    ...
    Performs ridge regression with K-group cross-validation and/or LOPO bootstrapping for alpha optimization.
    Combines group-based CV and alpha optimization into a single function with efficient group splitting.
    If n_groups equals the number of unique stories, it performs leave-one-participant-out CV.
    ...
    if optimize_alpha:
        ...
        np.random.seed(42)
        participant_choices = (np.random.choice(unique_stories, size=nboots, replace=True)
                              if with_replacement else
                              np.random.permutation(unique_stories)[:min(nboots, n_stories)])

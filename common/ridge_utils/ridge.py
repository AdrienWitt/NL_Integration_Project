...
def ridge_cv(stim, resp, alphas, story_ids,
             stim_test=None, resp_test=None,
             nboots=50, nsplits=50,
    ...
    """
    Ridge regression with LOO alpha optimisation and either:
      - K-fold CV to estimate encoding performance (final_test=False)
      - Final test on held-out story (final_test=True)

    Parameters
    ----------
    final_test : bool
        If False → run K-fold CV using nsplits, return corrs from CV folds.
        If True  → fit on all training data, predict on stim_test/resp_test,
                   return corrs from held-out story. Requires stim_test and resp_test.
    """
    stim = zs(stim) if normalize_stim else stim
    resp = zs(resp) if normalize_resp else resp

    if story_ids is None:
        raise ValueError("story_ids is required.")
    ...
    # ── Step 1: alpha optimisation (always runs unless valphas provided) ──
    if optimize_alpha:
        if nboots > n_stories and not with_replacement:
            raise ValueError(
                f"nboots ({nboots}) > n_stories ({n_stories}) without replacement."
            )
        logger.info(f"Alpha optimisation: {nboots} LOO iterations...") if logger else None

        np.random.seed(42)
        participant_choices = (
            np.random.choice(unique_stories, size=nboots, replace=True)
            if with_replacement
            else np.random.permutation(unique_stories)[:min(nboots, n_stories)]
        )

        def _bootstrap_iter(val_story, iteration, total):
            logger.info(f"  Bootstrap {iteration+1}/{total}: held-out={val_story}") if logger else None
            held     = story_ids == val_story
            not_held = ~held
            return ridge_corr(
                stim[not_held], stim[held],
                resp[not_held], resp[held],
                alphas,
        ...
        bootstrap_results = Parallel(n_jobs=n_jobs)(
            delayed(_bootstrap_iter)(s, i, nboots)
            for i, s in enumerate(participant_choices)
        )

        bootstrap_corrs = np.dstack(bootstrap_results)
        ...
        for ua in np.unique(valphas):
            sel = np.nonzero(valphas == ua)[0]
            mc  = np.mean(meanbootcorrs[bestalphainds[sel], sel])
            logger.info(f"  alpha={ua:.3f} → {len(sel)} voxels (mean corr={mc:.5f})") if logger else None

    else:
        if valphas is None:
            raise ValueError("valphas must be provided when optimize_alpha=False.")
        logger.info("Using precomputed valphas.") if logger else None

    # ── Step 2a: K-fold CV (final_test=False) ─────────────────────────────
    if not final_test:
        logger.info(f"K-fold CV: {nsplits} folds...") if logger else None
        nsplits = min(nsplits, n_stories)
        gkf     = GroupKFold(n_splits=nsplits)

        def _cv_iter(fold_idx, train_idx, test_idx):
            logger.info(f"  CV fold {fold_idx+1}/{nsplits}") if logger else None
            return ridge_corr_pred(
                stim[train_idx], stim[test_idx],
                resp[train_idx], resp[test_idx],
                valphas,
        ...
        fold_results = Parallel(n_jobs=n_jobs)(
            delayed(_cv_iter)(fi, tr, te)
            for fi, (tr, te) in enumerate(
                gkf.split(stim, resp, groups=story_ids)
            )
        )
        ...
        logger.info(
            f"CV done: mean r={np.mean(corrs):.5f}, max r={np.max(corrs):.5f}"
        ) if logger else None

    # ── Step 2b: final test (final_test=True) ────────────────────────────
    else:
        if stim_test is None or resp_test is None:
            raise ValueError(
                "stim_test and resp_test must be provided when final_test=True."
            )
        logger.info("Final test: fitting on all training data...") if logger else None
        ...
        fold_corrs = []
        logger.info(
            f"Final test done: mean r={np.mean(corrs):.5f}, max r={np.max(corrs):.5f}"
        ) if logger else None

    # ── Optional: regression weights ─────────────────────────────────────
    if return_wt:
        logger.info("Computing regression weights on full training data...") if logger else None
        wt = ridge(stim, resp, valphas,
                   singcutoff=singcutoff, normalpha=normalpha, logger=logger)

    return wt, corrs, valphas, fold_corrs, bootstrap_corrs, None
